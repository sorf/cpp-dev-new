...
int main() {
...

    {
        ...
        [[maybe_unused]] auto i1 = std::make_unique<std::uint16_t>(1);
        PRINT_ALLOCATIONS_AT_LINE();
        [[maybe_unused]] auto i2 = std::make_unique<std::uint8_t>(2);
        PRINT_ALLOCATIONS_AT_LINE();
    ...
    [[maybe_unused]] auto i1 = std::make_unique<std::uint8_t>(1);
    [[maybe_unused]] auto i2 = std::make_unique<std::uint8_t>(1);
    [[maybe_unused]] auto i3 = std::make_unique<std::uint8_t>(1);
    try {
        // This should fail again as we reach the amount of allocated memory at the time of the first generated error.
        [[maybe_unused]] auto c = std::make_unique<char>(0);
        std::cerr << "Error: We shouldn't get here !" << std::endl;
    } catch (std::exception &e) {
        dev_new::pause_error_testing();
        std::cout << "Expected error: " << e.what() << std::endl;
